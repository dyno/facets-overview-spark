...
class TensorConnectorTest extends AnyFunSuite with BeforeAndAfterAll {
  ...
  val exampleTestRows: Array[Row] = Array(
    new GenericRow(Array[Any](11, 1, 23L, 10.0f, 14.0, List(1.0, 2.0), "r1", Array[Byte](0xff.toByte, 0xf0.toByte))),
    new GenericRow(Array[Any](21, 2, 24L, 12.0f, 15.0, List(2.0, 2.0), "r2", Array[Byte](0xff.toByte, 0xf1.toByte)))
  )
  ...
  val sequenceExampleSchema = StructType(
    List(
  ...
  test("load tfrecords") {
    ...
    df.schema.foreach { f =>
      ...
      if (imported.isEmpty) {
        println(
          s"Field '${f.name}' with '${f.dataType}' is not found," +
...
//    df.printSchema()
    ...
    val importedDf: DataFrame =
      spark.read.format("tfrecords").option("recordType", "SequenceExample").schema(sequenceExampleSchema).load(path)
...
//    actualDf.printSchema()
  ...
  ignore("load yourtube video", Slow) {
    ...
    val videoDf: DataFrame = spark.read
      .format("tfrecords")
      .schema(videoSchema)
      .option("recordType", "Example")
    ...
    val importedDf1: DataFrame = spark.read
      .format("tfrecords")
      .option("recordType", "Example")
      .schema(videoSchema)
    ...
    val frameDf: DataFrame = spark.read
      .format("tfrecords")
      .schema(frameSchema)
    ...
    val importedDf2: DataFrame =
      spark.read.format("tfrecords").option("recordType", "SequenceExample").schema(frameSchema).load("youtube-8m-frame.tfrecord")
    importedDf2.show()
  }
