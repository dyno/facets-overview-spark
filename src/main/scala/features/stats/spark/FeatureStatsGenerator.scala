 ...
 *
 * the the first array of actors :
 *  ["Tim Robbins","Morgan Freeman"] are in first array of movies: ["The Shawshank Redemption"],["Fight Club"] with ratings [9.0]
 * the the 2nd array of actors :
 *  ["Brad Pitt","Edward Norton","Helena Bonham Carter"] are in 2nd array of movies: ["Fight Club"] with ratings [9.7]
 */
object FeatureStatsGenerator {
  ...
  private[features] def isFiniteFun(x: Any): Boolean = {
    val value = x.toString.toDouble // FIXME: why?
  ...
  private[features] def checkZeroNanInfiniteFun(x: AnyVal): NonZeroInfinite = {
    // FIXME: figure out why NPE...
    val value =
      try { x.toString.toDouble }
      catch { case e: NullPointerException => Float.NaN }
...
class FeatureStatsGenerator(datasetProto: DatasetFeatureStatisticsList) {
   ...
   */
  def protoFromDataFrames(
    dataFrames: List[NamedDataFrame],
    features: Set[String] = Set.empty[String],
    catHistgmLevel: Option[Int] = None
   ...
   */
  private[features] def genDatasetFeatureStats(
    datasets: List[DataEntrySet],
    features: Set[String] = Set.empty[String],
    catHistgmLevel: Option[Int] = None
  ): DatasetFeatureStatisticsList = {
    ...
    val dfsList: List[DatasetFeatureStatistics] = datasets.zipWithIndex.map { a =>
      ...
      val feat = ds.entries
        .filter(e => includeFeature(e.featureName))
        .map { entry =>
          val featureType = entry.`type`
          val fieldId = FeatureNameStatistics.FieldId.Name(entry.featureName)
          if (featureType == ProtoDataType.INT || featureType == ProtoDataType.FLOAT) {
            FeatureNameStatistics(fieldId = fieldId, `type` = featureType)
  ...
  private[features] def getCommonStats(dsSize: Long, entry: DataEntry, numNan: Long, dsIndex: Int): CommonStatistics = {
    ...
    val countQuantileHist = FSHistogram(numNan = numNan, buckets = countQuantileBuckets, `type` = FSHistogram.HistogramType.QUANTILES)

    val featureListLenHist: Option[FSHistogram] = entry.featLens.map { featLens =>
      val flQuantileBuckets = populateQuantilesHistogramBucket(featLens, dsIndex)
      FSHistogram(numNan = numNan, buckets = flQuantileBuckets, `type` = FSHistogram.HistogramType.QUANTILES)
    }
    ...
    val commStats = CommonStatistics(
      numNonMissing = dsSize - entry.missing,
  ...
  private[features] def getBasicNumStats(valueDF: DataFrame, colName: String, dsIndex: Int): BasicNumStats = {
    ...
    val filteredValueDF = valueDF.na
      .drop()
      .filter(!valueDF(colName).isNaN)
  ...
  private[features] def getBasicStringStats(valueDF: DataFrame, colName: String): BasicStringStats = {
    ...
    val aggDF = valueDF
      .map(row => (colName, isNaNFun(row.getAs(0)), 1L))
    ...
    val extraCounts: Map[String, Long] = result.flatMap(row => indexedColNames.map(f => f._1 -> row.getAs[Long](f._2))).toMap
  ...
  private[features] def updateStdHistogramBucket(colName: String, basicNumStats: BasicNumStats): Seq[FSHistogram.Bucket] = {
  ...
  private[features] def getNumericStats(entry: DataEntry, dsSize: Long, dsIndex: Int): NumericStatistics = {
    ...
    val stdHist = FSHistogram(numNan = basicStats.numNan, buckets = stdBuckets, `type` = FSHistogram.HistogramType.STANDARD)

    //val filteredValueDF = valueDF.filter(r => isFiniteFun(r.getAs(0)))
    val filteredValueDF = valueDF.na
      .drop()
      .filter(!valueDF(featureName).isNaN)
      .filter(r => isFiniteFun(r.getAs(0)))
    val qBuckets =
      if (filteredValueDF.rdd.isEmpty()) {
        Seq[FSHistogram.Bucket]()
      } else {
        populateQuantilesHistogramBucket(filteredValueDF, dsIndex)
      }

    val qHist = FSHistogram(numNan = basicStats.numNan, buckets = qBuckets, `type` = FSHistogram.HistogramType.QUANTILES)

    val commonStats: CommonStatistics = getCommonStats(dsSize, entry, basicStats.numNan, dsIndex)

    NumericStatistics(
      commonStats = Some(commonStats),
  ...
  private[features] def getStringStats(
    entry: DataEntry,
    dsSize: Long,
    dsIndex: Int,
    histgmCatLevelsCount: Option[Int]
  ): StringStatistics = {
    ...
    val ranksRdd = colDF.rdd
      .mapPartitions(it => it.map(r => (r.getAs[String](0), 1L)))
      .reduceByKey(_ + _)
      .sortBy(a => -1 * a._2)
  ...
  //Try to convert the data type to Googe Protobuf enum type
  private[features] def convertDataType(dfType: String): ProtoDataType =
    dfType.toLowerCase match {
      ...
      //remember to convert date to Int
      case x if x == "date" || x == "java.util.date" || x == "java.sql.date" || x == "java.sql.timestamp" => INT
      case _                                                                                              => STRING
    }
  ...
  private[features] def toRowCountDF(colDF: DataFrame): DataFrame = {
    ...
    val df = field.dataType match {
      ...
      case x: ArrayType   => colDF.select(size(colDF(field.name)))
      case _: NullType    => colDF.map(_ => 0)
      case s: NumericType => colDF.filter(!colDF(field.name).isNaN).map(_ => 1)
      case s: StringType  => colDF.withColumn(field.name, when(length(colDF(field.name)) > 0, 1).otherwise(lit(0)))
      case s: BooleanType => colDF.withColumn(field.name, when(colDF(field.name) === true, 1).otherwise(lit(0)))
